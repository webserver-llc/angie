...
#if (NGX_HAVE_CERT_ZLIB_COMPRESSION)
...
static ngx_int_t ngx_api_http_ssl_country_handler(ngx_api_entry_data_t data,
    ngx_api_ctx_t *actx, void *ctx);
static ngx_int_t ngx_api_http_ssl_state_or_province_handler(
    ngx_api_entry_data_t data, ngx_api_ctx_t *actx, void *ctx);
static ngx_int_t ngx_api_http_ssl_organization_handler(
    ngx_api_entry_data_t data, ngx_api_ctx_t *actx, void *ctx);
    ...
    ngx_array_t *keys, ngx_array_t *passwords)
{
    ...
    ngx_ssl_static_cert_t  *elm;

    oscf = (ngx_openssl_conf_t *) ngx_get_conf(cf->cycle->conf_ctx,
                                               ngx_openssl_module);
#endif
...
#if (NGX_API)
        elm = ngx_array_push(&oscf->static_certs);
        if (elm == NULL) {
            return NGX_ERROR;
        }
...
#elif (NGX_HAVE_CERT_ZLIB_COMPRESSION)
...
#if (NGX_API)
    if (ngx_array_init(&oscf->static_certs, cycle->pool, 4,
                       sizeof(ngx_ssl_static_cert_t))
        != NGX_OK)
    {
        return NULL;
    }
...
{
#if (NGX_API)
    if (ngx_api_add(cycle, "/", &ngx_api_http_ssl_entry)
        != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }
    ...
    void *ctx)
{
    ...
    ngx_array_t             certs;
    ngx_openssl_conf_t     *oscf;
    ngx_ssl_api_cert_t      elm, *elmp;
    ngx_api_iter_ctx_t      ictx;
    ngx_ssl_static_cert_t  *sc;
    ...
    for (i = 0; i < oscf->static_certs.nelts; i++) {

        sc = &((ngx_ssl_static_cert_t *) oscf->static_certs.elts)[i];
        cert = *sc->cert;
        key = *sc->key;

        elm.chain = ngx_ssl_cache_static_peek(actx->pool, NGX_SSL_CACHE_CERT,
                                              &cert);
        ...
        for (j = 0; j < certs.nelts; j++) {
            elmp = &((ngx_ssl_api_cert_t *) certs.elts)[j];

            if (elmp->chain == elm.chain) {
                /* skip duplicate certificates */
                goto next_cert;
            }
        }

        elm.pkey = ngx_ssl_cache_static_peek(actx->pool, NGX_SSL_CACHE_PKEY,
                                             &key);
        if (elm.pkey == NULL) {
            continue;
        }
...
#if (OPENSSL_VERSION_NUMBER < 0x40000000L)
    len = X509_NAME_get_text_by_NID((X509_NAME *) name, nid, (char *) buf,
                                    sizeof(buf));
#else
