...
int ngx_quic_select_socket_by_dcid(struct sk_reuseport_md *ctx)
{
    ...
    if (rc == 0) {
        debugmsg("angie quic socket selected by dcid");
        return SK_PASS;
    }

    if (rc != -ENOENT) {
        debugmsg("angie quic bpf_sk_select_reuseport() failed:%d", rc);
        /*
...
new_conn:
    ...
    debugmsg("angie quic new connection");
    ...
    if (val == NULL) {
         ...
         */
        debugmsg("angie quic master_state map is inconsistent");
        return SK_DROP;
    }

    debugmsg("angie quic master count %d", *val);

    if (*val == 2) {
        /* two masters, select randomly which to use */
        active = (bpf_get_prandom_u32() % 2);

        debugmsg("angie quic selected randomly master #%d", active);
        ...
        if (val == NULL) {
             ...
             * so just drop it
             */
            debugmsg("angie quic master_state map state is incosistent");
            return SK_DROP;
        }
        active = *val;

        debugmsg("angie quic selected active master: #%d", active);
    }
    ...
    if (val == NULL) {
        ...
        debugmsg("angie quic master_state map state is inconsistent");
        return SK_DROP;
    }

    nwrk = *val;
    debugmsg("  angie quic nworkers is %d", nwrk);
    ...
    if (lmap_ptr == NULL) {
         ...
         */
        debugmsg("angie quic listen_maps failed");
        return SK_DROP;
    }
    ...
    if (rc == 0) {
        debugmsg("angie quic socket selected by hash:%d of %d", (int) ns, nwrk);
        return SK_PASS;
    }

    /* again, our map is in inconsistent state, drop the packet */
    debugmsg("angie quic select_reuseport failed:%d", rc);

    return SK_DROP;

bad_dgram:

    debugmsg("angie quic bad datagram");
