...
ngx_http_v3_get_insert_buffer(ngx_connection_t *c)
{
    ...
    if (dt->insert_buffer == NULL) {
        ...
        dt->insert_buffer = ngx_create_temp_buf(c->pool,
                                           h3scf->settings.max_table_capacity);
        if (dt->insert_buffer == NULL) {
            return NULL;
        }
    }
...
ngx_http_v3_set_capacity(ngx_connection_t *c, ngx_uint_t capacity)
{
    ...
    if (capacity > h3scf->settings.max_table_capacity) {
...
ngx_http_v3_decode_insert_count(ngx_connection_t *c, ngx_uint_t *insert_count)
{
    ...
    max_entries = h3scf->settings.max_table_capacity / 32;
...
ngx_http_v3_check_insert_count(ngx_connection_t *c, ngx_uint_t insert_count)
{
    ...
    if (block->queue.prev == NULL) {
        ...
        if (h3c->nblocked == h3scf->settings.max_blocked_streams) {
            ngx_log_error(NGX_LOG_INFO, c->log, 0,
