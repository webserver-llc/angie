...
static char *ngx_stream_proxy_protocol_tlv(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
static ngx_inline ngx_int_t ngx_stream_complex_value_wrapper(void *ctx,
    void *cv, ngx_str_t *v);
...
#if (NGX_STREAM_PROXY_MULTICERT)
static ngx_int_t ngx_stream_proxy_ssl_certificates(ngx_stream_session_t *s);
#else
...
static ngx_command_t  ngx_stream_proxy_commands[] = {
    ...
    { ngx_string("proxy_protocol_version"),
      ...
      + offsetof(ngx_proxy_protocol_conf_t, version),
    ...
    { ngx_string("proxy_ssl_ntls"),
      ...
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, ssl_ntls),
      NULL },
#endif
...
ngx_stream_proxy_init_upstream(ngx_stream_session_t *s)
{
    ...
    if (u->proxy_protocol) {
        ...
        cl->buf->pos = ngx_proxy_protocol_write(c, &pscf->proxy_protocol_conf,
                                                &cl->buf->last);
        if (cl->buf->pos == NULL) {
            ngx_stream_proxy_finalize(s, NGX_STREAM_INTERNAL_SERVER_ERROR);
            return;
...
ngx_stream_proxy_send_proxy_protocol(ngx_stream_session_t *s)
{
    ...
    buf = ngx_proxy_protocol_write(c, &pscf->proxy_protocol_conf, &p);
    if (buf == NULL) {
        ngx_stream_proxy_finalize(s, NGX_STREAM_INTERNAL_SERVER_ERROR);
        return NGX_ERROR;
    }
...
ngx_stream_proxy_ssl_init_connection(ngx_stream_session_t *s)
{
    ...
    if (pscf->ssl_certificate_values) {
        if (ngx_stream_proxy_ssl_certificates(s) != NGX_OK) {
            ngx_stream_proxy_finalize(s, NGX_STREAM_INTERNAL_SERVER_ERROR);
            return;
        }
    }
...
static ngx_int_t
ngx_stream_proxy_ssl_certificates(ngx_stream_session_t *s)
{
    ...
    for (i = 0; i < nelts; i++) {
        ...
        if (ngx_stream_complex_value(s, &certs[i], certp) != NGX_OK) {
            return NGX_ERROR;
        }
        ...
        if (ngx_stream_complex_value(s, &keys[i], keyp) != NGX_OK) {
            return NGX_ERROR;
        }
    ...
    void *data)
{
    ...
    ngx_log_add_tag(log, ngx_stream_log_tag(UPSTREAM));

    if (u->peer.name) {
        ngx_log_add_tag(log, ngx_stream_log_tag(PEER_TAG));
...
ngx_stream_proxy_create_srv_conf(ngx_conf_t *cf)
{
    ...
    conf->proxy_protocol_conf.complex_value = ngx_stream_complex_value_wrapper;
    conf->proxy_protocol_conf.version = NGX_CONF_UNSET_UINT;
    conf->proxy_protocol_conf.tlvs = NGX_CONF_UNSET_PTR;
    conf->proxy_protocol_conf.tlvs_ssl = NGX_CONF_UNSET_PTR;
...
ngx_stream_proxy_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    if (conf->proxy_protocol_conf.tlvs == NGX_CONF_UNSET_PTR
        && conf->proxy_protocol_conf.tlvs_ssl == NGX_CONF_UNSET_PTR)
    {
        conf->proxy_protocol_conf.tlvs = prev->proxy_protocol_conf.tlvs;
        conf->proxy_protocol_conf.tlvs_ssl =
                                            prev->proxy_protocol_conf.tlvs_ssl;
    }
...
#if (NGX_HAVE_NTLS)
    ngx_conf_merge_value(conf->ssl_ntls, prev->ssl_ntls, 0);
#endif
...
ngx_stream_proxy_protocol_tlv(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    return ngx_proxy_protocol_conf_add_tlv(cf, &pscf->proxy_protocol_conf,
                                           &value[1], cv);
}
...
ngx_stream_complex_value_wrapper(void *ctx, void *cv, ngx_str_t *v)
{
