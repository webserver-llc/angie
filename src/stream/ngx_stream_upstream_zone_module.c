...
#if (NGX_DOCKER_UPSTREAM_SID_LEN != NGX_STREAM_UPSTREAM_SID_LEN)

#error NGX_DOCKER_UPSTREAM_SID_LEN and NGX_STREAM_UPSTREAM_SID_LEN are        \
    different!

#endif

static ngx_int_t ngx_stream_upstream_zone_add_docker_peer(
    ngx_docker_upstream_t *u);
static ngx_int_t ngx_stream_upstream_zone_remove_docker_peer(
    ngx_docker_upstream_t *u);
static ngx_int_t ngx_stream_upstream_zone_down_docker_peer(
    ngx_docker_upstream_t *u);
#endif
...
static ngx_int_t ngx_api_stream_upstream_peer_server_handler(
    ngx_api_entry_data_t data, ngx_api_ctx_t *actx, void *ctx);
static ngx_int_t ngx_api_stream_upstream_peer_service_handler(
    ngx_api_entry_data_t data, ngx_api_ctx_t *actx, void *ctx);
...
static ngx_int_t ngx_api_stream_upstream_peer_sid_handler(
    ngx_api_entry_data_t data, ngx_api_ctx_t *actx, void *ctx);
#endif
...
static ngx_api_entry_t  ngx_api_stream_upstream_peer_selected_entries[] = {

    {
        ...
        .handler   = ngx_api_stream_upstream_peer_struct_int_handler,
        .data.off  = offsetof(ngx_stream_upstream_rr_peer_t, conns)
    },

    {
        .name      = ngx_string("total"),
        .handler   = ngx_api_stream_upstream_peer_struct_int64_handler,
...
static ngx_api_entry_t  ngx_api_stream_upstream_peer_entries[] = {
...

    {
        ...
        .handler   = ngx_api_stream_upstream_peer_service_handler,
    },
    ...
    {
        .name      = ngx_string("sid"),
        .handler   = ngx_api_stream_upstream_peer_sid_handler,
    },
#endif
    ...
    ngx_resolver_addr_t *addr, ngx_stream_upstream_rr_peer_t *template)
{
    ...
    if (template->sid.len) {
        ...
        ngx_stream_upstream_rr_peer_init_sid(peer);
    }
#endif
    ...
    ngx_api_ctx_t *actx, void *ctx)
{
    ...
    ngx_int_t                            rc;
    ngx_api_iter_ctx_t                   ictx;
    ngx_api_stream_upstream_peers_ctx_t  peers_ctx;
    ...
    if (peers_ctx.locked) {
        ...
        ngx_stream_upstream_rr_peer_unlock(peers_ctx.peers, peers_ctx.peer);
        ngx_stream_upstream_rr_peers_unlock(peers_ctx.peers);
    }
...
ngx_api_stream_upstream_peers_iter(ngx_api_iter_ctx_t *ictx,
    ngx_api_ctx_t *actx)
{
    ngx_api_stream_upstream_peers_ctx_t  *pctx;

    pctx = ictx->ctx;

    if (pctx->peer == NULL) {
        ngx_stream_upstream_rr_peers_rlock(pctx->peers);
        pctx->peer = pctx->peers->peer;

    } else {
        ngx_stream_upstream_rr_peer_unlock(pctx->peers, pctx->peer);
    ...
    for ( ;; ) {
        ...
        if (pctx->peer == NULL) {
            ngx_stream_upstream_rr_peers_unlock(pctx->peers);
        ...
        ngx_stream_upstream_rr_peer_lock(pctx->peers, pctx->peer);
        pctx->locked = 1;
...
ngx_int_t
ngx_api_stream_upstream_peer_struct_int_handler(ngx_api_entry_data_t data,
    ngx_api_ctx_t *actx, void *ctx)
{
    ngx_api_stream_upstream_peers_ctx_t  *pctx = ctx;

    return ngx_api_struct_int_handler(data, actx, pctx->peer);
}


ngx_int_t
ngx_api_stream_upstream_peer_struct_int64_handler(ngx_api_entry_data_t data,
    ngx_api_ctx_t *actx, void *ctx)
{
    ngx_api_stream_upstream_peers_ctx_t  *pctx = ctx;
...
static ngx_int_t
ngx_api_stream_upstream_peer_service_handler(ngx_api_entry_data_t data,
    ngx_api_ctx_t *actx, void *ctx)
{
    ngx_api_stream_upstream_peers_ctx_t  *pctx = ctx;
    ...
    ngx_api_ctx_t *actx, void *ctx)
{
    ...
    } else if (ngx_stream_upstream_rr_is_busy(peer)) {
...
static ngx_int_t
ngx_api_stream_upstream_peer_sid_handler(ngx_api_entry_data_t data,
    ngx_api_ctx_t *actx, void *ctx)
{
    ngx_api_stream_upstream_peers_ctx_t  *pctx = ctx;
...
#if (NGX_DOCKER)
    ngx_docker_init_stream_upstream_actions(
                                  &ngx_stream_upstream_zone_add_docker_peer,
                                  &ngx_stream_upstream_zone_remove_docker_peer,
                                  &ngx_stream_upstream_zone_down_docker_peer);
#endif
...
ngx_stream_upstream_zone_resolve_timer(ngx_event_t *event)
{
    ...
    if (template->zombie) {
        ...
        ngx_stream_upstream_rr_peer_lock(peers, template);

        if (ngx_stream_upstream_rr_peer_unref(peers, template) == NGX_OK) {
            ngx_stream_upstream_rr_peer_unlock(peers, template);
        }
...
ngx_stream_upstream_zone_get_srv_conf(ngx_docker_upstream_t *u, ngx_log_t *log)
{
    ngx_uint_t                        i;
    ngx_stream_upstream_srv_conf_t   *uscf, **uscfp;
    ngx_stream_upstream_main_conf_t  *umcf;
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        if (uscf->shm_zone == NULL) {
            ngx_log_error(NGX_LOG_ERR, log, 0,
                          ...
                          "stream upstream \"%V\" should be configured with a "
                          "shared memory zone",
                          &u->container->id, &u->name);
            return NULL;
        }

        if (!(uscf->flags & NGX_STREAM_UPSTREAM_CONF)) {
            ngx_log_error(NGX_LOG_ERR, log, 0,
                          "processing Docker container \"%V\" failed: "
                          "modification of the peers is not possible in stream "
...
static ngx_int_t
ngx_stream_upstream_zone_add_docker_peer(ngx_docker_upstream_t *u)
{
    ngx_resolver_addr_t              addr;
    ngx_docker_container_t          *dc;
    ngx_stream_upstream_host_t       host;
    ngx_stream_upstream_rr_peer_t   *peer, **peerp, template;
    ngx_stream_upstream_rr_peers_t  *peers;
    ngx_stream_upstream_srv_conf_t  *uscf;

    dc = u->container;

    uscf = ngx_stream_upstream_zone_get_srv_conf(u, dc->pool->log);
    if (uscf == NULL) {
        return NGX_ERROR;
    }

    if (u->container->down && !(uscf->flags & NGX_STREAM_UPSTREAM_DOWN)) {
        return NGX_OK;
    }

    if (u->backup && !(uscf->flags & NGX_STREAM_UPSTREAM_BACKUP)) {
        u->backup = 0;
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      "\"backup\" is not supported in stream upstream \"%V\" "
                      "with the selected balancing method",
                      &u->name);
    }

    if (u->weight != (ngx_uint_t) -1
        && !(uscf->flags & NGX_STREAM_UPSTREAM_WEIGHT))
    {
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      "\"weight\" is not supported in stream upstream \"%V\" "
                      "with the selected balancing method",
                      &u->name);
    }

    if (u->max_fails != (ngx_uint_t) -1
        && !(uscf->flags & NGX_STREAM_UPSTREAM_MAX_FAILS))
    {
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      "\"max_fails\" is not supported in stream "
                      "upstream \"%V\" with the selected balancing method",
                      &u->name);
    }

    if (u->max_conns != (ngx_uint_t) -1
        && !(uscf->flags & NGX_STREAM_UPSTREAM_MAX_CONNS))
    {
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      "\"max_conns\" is not supported in stream "
                      "upstream \"%V\" with the selected balancing method",
                      &u->name);
    }

    if (u->fail_timeout != (time_t) -1
        && !(uscf->flags & NGX_STREAM_UPSTREAM_FAIL_TIMEOUT))
    {
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      "\"fail_timeout\" is not supported in stream "
                      "upstream \"%V\" with the selected balancing method",
                      &u->name);
    }

    if (u->slow_start != (ngx_msec_t) -1
        && !(uscf->flags & NGX_STREAM_UPSTREAM_SLOW_START))
    {
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      "\"slow_start\" is not supported in stream"
                      "upstream \"%V\" with the selected balancing method",
                      &u->name);
    }

    ngx_memzero(&host, sizeof(ngx_stream_upstream_host_t));
    ngx_memzero(&template, sizeof(ngx_stream_upstream_rr_peer_t));
    ...
    if (u->sid.len != 0) {
        ...
        ngx_log_error(NGX_LOG_WARN, dc->pool->log, 0,
                      ...
                      "Angie was built without stream upstream sticky module "
                      "(--without-stream_upstream_sticky_module)",
    ...
    *peerp = peer;
    ...
    ngx_log_error(NGX_LOG_NOTICE, dc->pool->log, 0,
                  "Docker peer \"%V\" was added to stream upstream \"%V\"",
                  &u->url.url, &u->name);
...
static ngx_int_t
ngx_stream_upstream_zone_remove_docker_peer(ngx_docker_upstream_t *u)
{
    ngx_docker_container_t          *dc;
    ngx_stream_upstream_rr_peer_t   *peer, **peerp;
    ngx_stream_upstream_rr_peers_t  *peers;
    ...
    for (peerp = &peers->peer; *peerp; peerp = &(*peerp)->next) {
        ...
        *peerp = peer->next;
        ...
        ngx_log_error(NGX_LOG_NOTICE, dc->pool->log, 0,
                      "Docker peer \"%V\" was removed from stream "
                      "upstream \"%V\"",
                      &u->url.url, &u->name);
    ...
    ngx_log_error(NGX_LOG_ALERT, dc->pool->log, 0,
                  "cannot find Docker peer \"%V\" in stream upstream \"%V\"",
                  &u->url.url, &u->name);
...
static ngx_int_t
ngx_stream_upstream_zone_down_docker_peer(ngx_docker_upstream_t *u)
{
    ngx_docker_container_t        *dc;
    ngx_stream_upstream_rr_peer_t   *peer, **peerp;
    ngx_stream_upstream_rr_peers_t  *peers;
    ...
    if (!(uscf->flags & NGX_STREAM_UPSTREAM_DOWN)) {
        return dc->down ? ngx_stream_upstream_zone_remove_docker_peer(u)
                        : ngx_stream_upstream_zone_add_docker_peer(u);
    }
