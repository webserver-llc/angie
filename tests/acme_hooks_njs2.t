...
# the ngx.fetch method to control the challenge server.
...
//    await sleep(0);
    ...
    if (hook == 'add') {
        if (challenge == 'http') {
            ngx.fetch('http://127.0.0.1:$challtestsrv_mgmt_port/add-http01',
                        {
            ...
            ngx.fetch('http://127.0.0.1:$challtestsrv_mgmt_port/set-txt',
                        {
            ...
            ngx.fetch('http://127.0.0.1:$challtestsrv_mgmt_port/add-tlsalpn01',
                        {
        ...
        if (challenge == 'http') {
            ngx.fetch('http://127.0.0.1:$challtestsrv_mgmt_port/del-http01',
                        {
                            body: `{"token":"\${token}."}`,
                            method: "POST"
                        })
            ...
            ngx.fetch('http://127.0.0.1:$challtestsrv_mgmt_port/clear-txt',
                        {
            ...
            ngx.fetch('http://127.0.0.1:$challtestsrv_mgmt_port/del-tlsalpn01',
                        {
                            body: `{"host":"\${domain}"}`,
                            method: "POST"
                        })
