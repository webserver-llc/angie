...
# Tests for upstream max_conns directive statistics.
...
my $t = Test::Nginx->new()->has(qw/http proxy upstream_zone http_api/)
	->plan(4);
...
my @sockets = http_get_multi('/' . $upstream, $count);

my $api = get_json("/api/status/http/upstreams/$upstream/peers");
...
$api = get_json("/api/status/http/upstreams/$upstream/peers");
