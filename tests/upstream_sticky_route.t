...
# if there are alive primary servers, backup server should still
# be accessible by id
is_sticky('/sticky_route_backup_alive', 'sticky=2', 'cookie', 2,
	'backup in alive upstream');

# we tried to select primary peer by id, but it turned out to be dead,
# so we # fall back to the first available backup peer
# without taking its id into account
like(http_get('/sticky_route_backup_same?sticky=1'), qr/backend_0/,
	'first backup server answered - sticky is not favored');
